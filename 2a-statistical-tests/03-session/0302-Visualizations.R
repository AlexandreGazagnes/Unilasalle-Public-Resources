...
##  Creatiing a Matrix and a Layout
##################################################
...
# We can create a new one dimensionnal array
ser = rnorm(100000,0,1)


# Using a xlim to specify the range -4 to 4
hist(ser ,main=title, xlab=" X ", ylab="Occurrences", col="blue",labels=TRUE, 
breaks=10, xlim=c(-4, 4))


# Using a xlim to specify the range -10 to 10
hist(ser ,main=title, xlab=" X ", ylab="Occurrences", col="blue",labels=TRUE, 
...
# Adding a 1 dimensionnal layout
layout(1)


# Lets create a new numerical vector
...
# specifiying the axis
axis(side=1,at=seq(-4,4,1))


# Using x/y labels and breaks
hist(v,main="Teacher size estimation",xlab="Taille (m)",ylab="Occurences",breaks=seq(1.65,1.95,.01))


# Adding some facny filling patern in the histogram
...
# Boxplot with specific values and orientation
